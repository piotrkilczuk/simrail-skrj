...
def build_fodts():
    for train_group_definition in train_definitions.registry:
        for train in train_group_definition:
            ...
            text_out = train_group_definition.template.replace("{{ train_category }}", train.category)
            text_out = text_out.replace("{{ train_number }}", str(train.number))
            for hour in HOUR_RE.findall(text_out):
                ...
                print(hour, hour_adjusted_as_str)
